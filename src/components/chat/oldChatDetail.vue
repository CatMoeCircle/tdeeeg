...
const getChatStatus = () => {
    ...
    // Simple status logic
    if (chat.value.type._ === 'chatTypePrivate') return t('lng_media_auto_private_chats');
    if (chat.value.type._ === 'chatTypeBasicGroup') return t('lng_notification_groups');
    if (chat.value.type._ === 'chatTypeSupergroup') {
        return chat.value.type.is_channel ? t('lng_notification_channels') : '超级群组';
    }
    return '';
};
