...
export function isSelfMessage(msg: message, deps: SelfDeps): boolean {
  const isChannel =
    deps.chat?.type?._ === "chatTypeSupergroup" &&
 ...
 * @returns 标签文本
 */
export function getSenderRoleText(
  sender: MessageSender | undefined,
  ctx: RoleContext,
): string {
 ...
 */
export function getSenderRoleClass(
  sender: MessageSender | undefined,
  ctx: RoleContext,
): string {
  const role = getSenderRole(sender, ctx);
...
export function isLinkedChannelMessage(msg: message): boolean {
  ...
  return (
    origin._ === "messageOriginChannel" &&
    origin.chat_id === msg.sender_id.chat_id
  );
}
...
export function getMessageLabel(msg: message, ctx: RoleContext): string {
  if (isLinkedChannelMessage(msg))
    return i18n.global.t("lng_notification_channels");
 ...
 * @returns 展示用的转发信息
 */
export function getDisplayForwardInfo(
  msg: message,
): messageForwardInfo | undefined {
...
export function senderUserId(msg: message): number | undefined {
  return msg.sender_id?._ === "messageSenderUser"
...
export function getInlineKeyboard(
  msg: message,
): replyMarkupInlineKeyboard | undefined {
  return msg.reply_markup?._ === "replyMarkupInlineKeyboard"
    ? msg.reply_markup
    : undefined;
}
...
export function getDisplayAuthorSignature(msg: message): string | undefined {
  ...
  return msg.forward_info
    ? getForwardAuthorSignature(msg.forward_info)
    : undefined;
}
