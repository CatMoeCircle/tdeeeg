...
export function useStickerPicker(opts: {
    ...
    function allGroups(): StickerGroup[] {
        ...
        if (favorites.value.length) {
            groups.push({ key: 'favorites', title: i18n.global.t('lng_saved_short'), lazy: false, size: favorites.value.length, stickers: favorites.value, setId: '', isFavorites: true });
        }
        if (recent.value.length) {
            groups.push({ key: 'recent', title: i18n.global.t('lng_recent_title'), lazy: false, size: recent.value.length, stickers: recent.value, setId: '' });
        }
        // 当前会话的群组贴纸包
