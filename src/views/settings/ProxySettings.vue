<template>
    <div class="h-full flex flex-col bg-white dark:bg-gray-900">
        <div class="p-4 border-b border-gray-200 dark:border-gray-800 flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold">{{ t('lng_proxy_settings') }}</h2>
        </div>
            ...
            <div class="max-w-2xl">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-colors"
                            ...
                            <div class="flex items-center">
                                <div
                                    class="w-9 h-9 rounded-full flex items-center justify-center mr-3 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600">
                                    <SmartphoneIcon class="w-5 h-5" />
                                </div>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100">{{
                                        t('proxy.autoFollowSystem') }}</h4>
                        ...
                        <div class="flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-colors"
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100">{{
                                        t('lng_proxy_use_system_settings') }}</h4>
                                    <p class="text-xs text-gray-400 mt-0.5">
                                        {{ systemProxyText }}
                                    </p>
                                    <p v-if="settings.proxy.mode === 'system' && !systemProxy"
                                        class="text-xs text-amber-500 mt-1">{{ t('proxy.systemProxyMissing') }}</p>
                                </div>
                            </div>
                            <div class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0"
                        ...
                        <div class="p-4 rounded-xl border cursor-pointer transition-colors" :class="settings.proxy.mode === 'custom'
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <h4 class="text-sm font-medium text-gray-900 dark:text-gray-100">{{
                                            t('lng_proxy_use') }}</h4>
                                        <p class="text-xs text-gray-400 mt-0.5">{{ t('proxy.useFromListDesc') }}</p>
                                    </div>
                                </div>
                                <div class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0"
                            ...
                            <div v-if="settings.proxy.mode === 'custom'"
                                ...
                                <div v-else class="flex items-center gap-2 text-sm">
                                    <span class="text-gray-500 dark:text-gray-400">{{ t('proxy.currentlyUsing')
                                    }}</span>
                ...
                <div>
                    <div class="flex items-center gap-3 mb-1">
                        ...
                        <button type="button" @click="addVisible = true"
                    ...
                    <!-- 加载中 -->
                    <div v-if="proxiesLoading" class="py-6 flex items-center justify-center text-sm text-gray-400">
                        {{ t('proxy.loadingList') }}</div>

                    <!-- 加载失败 -->
                    <div v-else-if="proxiesError" class="py-6 flex flex-col items-center justify-center gap-2 text-sm">
                    ...
                    <div v-else-if="proxies.length === 0"
                        ...
                        <p>{{ t('proxy.emptyList') }}</p>
                        <p class="text-xs">{{ t('proxy.emptyListHint') }}</p>
                    </div>
                    ...
                    <div v-else class="space-y-2">
                        <div v-for="p in proxies" :key="p.id" @click="useProxy(p)"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">{{ t('proxy.portLabel', { port: p.proxy.port })
                                }}</p>
                            </div>
                            ...
                            <button type="button" @click.stop="pingProxy(p)" :disabled="pinging"
                                class="shrink-0 text-gray-400 hover:text-cyan-500 transition-colors disabled:opacity-50"
                            ...
                            <button type="button" @click.stop="shareProxy(p)"
                                class="shrink-0 text-gray-400 hover:text-cyan-500 transition-colors"
                                :title="t('proxy.shareLink')">
        ...
        <!-- 添加代理弹窗（添加成功后组件内已同步 custom 模式；列表由 addProxyTo 刷新） -->
        <AddProxyDialog v-model="addVisible" />
    </div>
</template>
...
import {
    BanIcon, ChevronLeft as ChevronLeftIcon, MonitorIcon, ServerIcon, TrashIcon, RefreshCwIcon, PlusIcon, PlayIcon, Share2Icon, CheckIcon, SmartphoneIcon, ActivityIcon,
...
import AddProxyDialog from "../../components/settings/AddProxyDialog.vue";
...
const systemProxyText = computed(() => {
    if (systemProxy.value) {
        return t('proxy.systemProxyCurrent', {
            server: systemProxy.value.server,
            port: systemProxy.value.port,
        });
    }
    return t('proxy.systemProxyRead');
});
...
/** 切换代理模式，并向 Rust 应用 */
async function selectMode(mode: "auto" | "disabled" | "system" | "custom") {
...
async function shareProxy(p: addedProxy) {
    ...
    if (!link) {
        MessagePlugin.warning(t('proxy.shareUnavailable'));
        return;
    }
    try {
        await navigator.clipboard.writeText(link);
        MessagePlugin.success(t('proxy.shareCopied'));
    } catch (e: any) {
        MessagePlugin.error(t('proxy.copyFailed'));
        console.error(e);
    }
}
